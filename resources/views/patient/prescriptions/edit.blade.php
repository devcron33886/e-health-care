        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.prescriptions.update", [$prescription->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('patient') ? 'is-invalid' : '' }}" name="patient_id" id="patient_id" required>
                        @foreach($patients as $id => $entry)
                            <option value="{{ $id }}" {{ (old('patient_id') ? old('patient_id') : $prescription->patient->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('medic_one') ? 'is-invalid' : '' }}" type="text" name="medic_one" id="medic_one" value="{{ old('medic_one', $prescription->medic_one) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('medic_two') ? 'is-invalid' : '' }}" type="text" name="medic_two" id="medic_two" value="{{ old('medic_two', $prescription->medic_two) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('medic_three') ? 'is-invalid' : '' }}" type="text" name="medic_three" id="medic_three" value="{{ old('medic_three', $prescription->medic_three) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('medic_four') ? 'is-invalid' : '' }}" type="text" name="medic_four" id="medic_four" value="{{ old('medic_four', $prescription->medic_four) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('doctor') ? 'is-invalid' : '' }}" name="doctor_id" id="doctor_id" required>
                        @foreach($doctors as $id => $entry)
                            <option value="{{ $id }}" {{ (old('doctor_id') ? old('doctor_id') : $prescription->doctor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
